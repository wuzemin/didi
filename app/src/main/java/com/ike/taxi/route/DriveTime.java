 ...
 * 封装驾车路径规划
 */
public class DriveTime implements RouteSearch.OnRouteSearchListener{
    private Context context;
    private static DriveTime mRouteTask;
    ...
    private List<OnRouteCalculateListener> mListeners = new ArrayList<DriveTime.OnRouteCalculateListener>();
    ...
    public static DriveTime getInstance(Context context, AMap aMap) {
        if (mRouteTask == null) {
            mRouteTask = new DriveTime(context,aMap);
        }
        return mRouteTask;
    }

    public DriveTime(Context context, AMap aMap) {
        this.aMap=aMap;
            ...
            if(null!=driveRouteResult && null!=driveRouteResult.getPaths()){
                if(driveRouteResult.getPaths().size()>0){
                    for (OnRouteCalculateListener listener : mListeners) {
                        ...
                        listener.onRouteTime(taxiCost, dis, dur);
                    }
